...
int main(int argc, char* argv[]) {
	if (argc < 3) {
		...
		std::cout << "Example: " << argv[0]
				<< " ~/vms_connectors/my_vms_connector my_vms_connector_plugin.so" << std::endl;
		std::cout << "Provide VMS information using ./vms_connector.config" << std::endl;
		return -1;
	}
	std::string vms_connector_plugin_dir_path = argv[1];
	std::string vms_connector_plugin_filename = argv[2];

	opencctv::ConcurrentQueue<opencctv::Image>* pQueue = new opencctv::ConcurrentQueue<opencctv::Image>(10);
	...
	try {
		...
		std::cerr << "Failed to read VMS Connector Configuration file. " << e.what() << std::endl;
		return -1;
	}
	// Load VMS info from config file
	api::VmsConnectInfo vmsInfo;
	vmsInfo.sServerIp = pConfig->get(PROPERTY_SERVER_IP);
	vmsInfo.iServerPort = boost::lexical_cast<unsigned int>(pConfig->get(PROPERTY_SERVER_PORT));
	vmsInfo.sUsername = pConfig->get(PROPERTY_USERNAME);
	vmsInfo.sPassword = pConfig->get(PROPERTY_PASSWORD);
	vmsInfo.iWidth = boost::lexical_cast<unsigned int>(pConfig->get(PROPERTY_WIDTH));
	vmsInfo.iHeight = boost::lexical_cast<unsigned int>(pConfig->get(PROPERTY_HEIGHT));
	vmsInfo.sCameraId = pConfig->get(PROPERTY_CAMERA_ID);
	vmsInfo.bKeepAspectRatio = boost::lexical_cast<bool>(pConfig->get(PROPERTY_KEEP_ASPECT_RATIO));
	vmsInfo.bAllowUpsizing = boost::lexical_cast<bool>(pConfig->get(PROPERTY_ALLOW_UPSIZING));
	vmsInfo.iCompressionRate = boost::lexical_cast<unsigned int>(pConfig->get(PROPERTY_COMPRESSION_RATE));
	...
	try {
		...
		std::cout << "VMS connector plugin loaded. " << sPathToVmsConnectorPlugin << std::endl;
		pVmsConnector = loader.createPluginInstance();
		std::cout << "VMS connector instance created. " << std::endl;
	} catch (opencctv::Exception &e) {
		std::cerr << "Failed to load the Analytic plugin: " << sPathToVmsConnectorPlugin << " Error: " << e.what()
				<< std::endl;
		return -1;
	}
	...
	try {
		if (pVmsConnector->init(vmsInfo, vms_connector_plugin_dir_path)) {
			...
			std::cout << "VMS connector produceImageObjects started. " << std::endl;
			pVmsConnector->produceImageObjects(pQueue);
		} else {
			std::cerr << "Failed to initialize the VMS connector. init() failed.";
		}
